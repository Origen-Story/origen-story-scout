<script>
  ...
  const CATEGORY_GROUPS = {
    'AI Film': ['AI Film & Creative', 'AI YouTube'],
    'Climate': ['Climate Future', 'Climate & Clean Energy', 'Climate Podcasts'],
    'C2PA': ['Content Authenticity'],
    'AI News': ['AI - General News', 'AI Labs & Research', 'AI News & Commentary', 'AI Research', 'AI Podcasts']
  };
  ...
  function getSourceType(story) {
    const category = story.source_category || '';
    if (category === 'Newsletter') return { type: 'newsletter', icon: '📧', label: 'Newsletter' };
    if (category === 'AI YouTube') return { type: 'youtube', icon: '▶', label: 'YouTube' };
    if (category === 'GitHub Releases') return { type: 'github', icon: '⚙', label: 'GitHub' };
    if (category.includes('Podcast')) return { type: 'podcast', icon: '🎙', label: 'Podcast' };
    // Check metadata for YouTube videos from RSS
    if (story.is_youtube) return { type: 'youtube', icon: '▶', label: 'YouTube' };
    if (story.is_podcast) return { type: 'podcast', icon: '🎙', label: 'Podcast' };
    return { type: 'rss', icon: '📰', label: 'RSS' };
  }
      ...
      <div class="nav-tabs">
        ...
        {#each Object.keys(CATEGORY_GROUPS) as categoryName}
          <button
            ...
            class:empty={categoryCounts[categoryName] === 0}
            onclick={() => { activeCategory = categoryName; activeFilter = null; }}
          >
            {categoryName}
            <span class="tab-count">{categoryCounts[categoryName] || 0}</span>
          </button>
        {/each}
      ...
      <div class="story-grid">
        {#each filteredStories.slice(0, 9) as story}
          <article class="story-card">
            ...
            <span class="story-meta">{getSourceType(story).label}{#if story.published_date} - {formatDateShort(story.published_date)}{/if}</span>
        ...
        <section class="secondary-section">
          ...
          <div class="headline-list">
            {#each filteredStories.slice(9) as story}
              {@const sourceType = getSourceType(story)}
              <div class="headline-item">
                ...
                <div class="headline-meta">
                  ...
                  <span class="story-date">{sourceType.label}{#if story.published_date} - {formatDateShort(story.published_date)}{/if}</span>
                  <button
