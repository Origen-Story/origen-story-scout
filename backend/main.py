...
from backend.ingestion.rss import RSSSource
from shared.base import ContentItem
# from backend.scoring.provenance import ProvenanceVerifier  # Disabled - too slow without real C2PA sources
from backend.scoring.scorer import Scorer
from backend.summarizer.summarizer import Summarizer
from backend.scoring.trending import detect_trending, apply_trending_boost
from shared.report import ReportGenerator
from shared.storage.archive import Archive
...
def fetch_gmail_newsletters() -> list:
    ...
    try:
        from backend.ingestion.gmail import GmailSource, GMAIL_AVAILABLE
...
@click.option('--summarize/--no-summarize', default=None, help='Generate AI summaries (uses API tokens). Default: uses config setting.')
...
def run(limit, summarize, force, dev, report_only, skip_ingest):
    ...
    if report_only:
        ...
        report_path = Path("frontend/public/data/latest_report.json")
    ...
    console.print("[yellow]Phase 4: Keyword Scoring...[/yellow]")

    # Score ALL items with keyword scoring
    ...
    # Determine whether to summarize: CLI flag overrides config setting
    should_summarize = summarize if summarize is not None else settings.interests.preferences.auto_summarize
    num_to_summarize = settings.interests.preferences.summary_count
    ...
    if should_summarize and top_items:
